...
describe VCSToolkit::Objects::Label do
  ...
  let(:label) { described_class.new label_name, reference_id }
  ...
  it 'should equal other labels with the same data' do
    label_two = described_class.new label_name, reference_id
  ...
  it 'should not equal labels with different data' do
    label_two   = described_class.new label_name, 'other_ref_id'
    label_three = described_class.new 'master',   reference_id
